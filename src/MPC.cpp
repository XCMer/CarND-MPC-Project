...
public:
    ...
    void operator()(ADvector &fg, const ADvector &vars) {
        ...
        // Cost based on reference
        // Reference values for cte and epsi are 0,
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
    ...
    // Decompose state into individual variables
    double x = state[0];
    double y = state[1];
    double psi = state[2];
    double v = state[3];
    ...
    // 2 actuators (a, delta)
    size_t n_vars = no_states * N + no_actuators * (N - 1);

    // TODO: Set the number of constraints
    size_t n_constraints = no_states * N;
    ...
    // Max values on either side for non-actuators
    for (int i = 0; i < delta_start; i++) {
        vars_lowerbound[i] = -1.0e19; // Max lower
        vars_upperbound[i] = 1.0e19; // Max higher
    }

    // -25 to +25 degrees for steering
    for (int i = delta_start; i < a_start; i++) {
        vars_lowerbound[i] = -0.436332;
        vars_upperbound[i] = 0.436332;
    }

    // -1.0 to +1.0 for acceleration
    ...
    // +1 because the first element is the cost function
